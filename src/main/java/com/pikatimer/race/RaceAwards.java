...
public class RaceAwards {
    ...
    private final ObservableList<AwardCategory> awardCategoriesList = FXCollections
            .observableArrayList(AwardCategory.extractor());
    ...
    @Id
    @GenericGenerator(name = "race_awards_generator", strategy = "foreign", parameters = @Parameter(name = "property", value = "race"))
    ...
    public void setAwardCategories(List<AwardCategory> a) {
        ...
        if (awardCategories != null) {
            for (AwardCategory ac : awardCategories) {
                ac.setRaceAward(this);
            }
            if (awardCategoriesList.isEmpty()) {
                awardCategoriesList.addAll(awardCategories);
            }
        }
    }
    ...
    public Map<AwardCategory, Map<String, List<AwardWinner>>> getAwardWinners(List<ProcessedResult> pr) {
        ...
        List<ProcessedResult> contendersList = new ArrayList(
                pr.stream().filter(p -> p.getChipFinish() != null).filter(p -> p.getParticipant().getDNF() != true)
                        .filter(p -> p.getParticipant().getDQ() != true).filter(p -> {
                            if (cutoffTime.isZero())
                                return true;
                            return cutoffTime.compareTo(p.getChipFinish()) >= 0 || cutoffTimeString.equals(
                                    DurationFormatter.durationToString(p.getChipFinish(), dispFormat, roundMode));
                        }).sorted((p1, p2) -> p1.getChipFinish().compareTo(p2.getChipFinish()))
                        .collect(Collectors.toList()));

        for (int i = 0; i < awardCategoriesList.size(); i++) {
            Pair<Map<String, List<AwardWinner>>, List<ProcessedResult>> results = awardCategoriesList.get(i)
                    .process(contendersList);
    ...
    public void createDefaultCategories() {
        ...
        if (attributes != null && !attributes.isEmpty()) {
            ...
            if (attributes.containsKey("OverallMaleDepth"))
                o.setDepth(Integer.parseInt(attributes.get("OverallMaleDepth")));
            if (attributes.containsKey("OverallChip"))
                o.setChip(Boolean.parseBoolean(attributes.get("OverallChip")));
            if (attributes.containsKey("OverallPull"))
                o.setPull(Boolean.parseBoolean(attributes.get("OverallPull")));

            if (attributes.containsKey("MastersMaleDepth"))
                o.setDepth(Integer.parseInt(attributes.get("MastersMaleDepth")));
            if (attributes.containsKey("MastersChip"))
                m.setChip(Boolean.parseBoolean(attributes.get("MastersChip")));
            if (attributes.containsKey("MastersPull"))
                m.setPull(Boolean.parseBoolean(attributes.get("MastersPull")));

            if (attributes.containsKey("AGMaleDepth"))
                ag.setDepth(Integer.parseInt(attributes.get("AGMaleDepth")));
            if (attributes.containsKey("AGChp"))
                ag.setChip(Boolean.parseBoolean(attributes.get("AGChp")));
