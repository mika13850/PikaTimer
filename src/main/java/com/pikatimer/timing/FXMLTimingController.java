...
public class FXMLTimingController {
    ...
    @FXML
    private Button timingLocRemoveAllButton;
    @FXML
    ...
    @FXML
    private VBox timingLocVBox;
    @FXML
    private TextField timingLocationNameTextField;

    @FXML
    private TextField filterStartTextField;
    @FXML
    private TextField filterEndTextField;

    @FXML
    private TextField searchTextBox;
    @FXML
    private CheckComboBox<TimingLocation> searchScopeCheckComboBox;
    @FXML
    private Label listSizeLabel;
    @FXML
    private Label filteredSizeLabel;
    @FXML
    private TableView<CookedTimeData> timeTableView;
    @FXML
    private TableColumn bibColumn;
    @FXML
    private TableColumn<CookedTimeData, String> chipColumn;
    @FXML
    private TableColumn timeColumn;
    @FXML
    private TableColumn<CookedTimeData, String> nameColumn;
    @FXML
    private TableColumn<CookedTimeData, String> locationColumn;
    @FXML
    private TableColumn<CookedTimeData, String> inputColumn;
    @FXML
    private TableColumn<CookedTimeData, Boolean> backupColumn;
    @FXML
    private TableColumn<CookedTimeData, Boolean> ignoreColumn;
    @FXML
    private CheckBox customChipBibCheckBox;

    @FXML
    private ListView<TimeOverride> overrideListView;
    ...
    @FXML
    private Button overrideEditButton;
    @FXML
    private Button overrideRemoveButton;

    @FXML
    private ToggleSwitch announcerToggleSwitch;
    @FXML
    private ToggleSwitch assignToRaceToggleSwitch;
    // @FXML private ChoiceBox<Race> assignToRaceChoiceBox;
    @FXML
    private ComboBox<Race> assignToRaceComboBox;
    ...
    @FXML
    private VBox timingDetailsVBox;
    ...
    public void initialize() {
        ...
        timingLocRemoveButton.disableProperty()
                .bind(timingLocListView.getSelectionModel().selectedItemProperty().isNull());
        ...
        searchScopeCheckComboBox.getCheckModel().getCheckedItems()
                .addListener((ListChangeListener.Change<? extends TimingLocation> c) -> {
        ...
        nameColumn.setCellValueFactory(cellData -> {
            ...
            if (p == null) {
                if (bib.startsWith("Unmapped"))
                    return new SimpleStringProperty("Unknown Chip");
                if (cellData.getValue().getRawChipID().equals("0"))
                    return new SimpleStringProperty("START TRIGGER");
        ...
        ///
        // FIX THIS if we permit multiple select and a right mouse click to set the
        /// ignored state
        ...
        sortedTimeList.addListener((ListChangeListener.Change<? extends CookedTimeData> c) -> {
            while (c.next()) {
                if (c.wasUpdated()) {
                    ...
                    logger.debug("CookedTimeData " + sortedTimeList.get(c.getFrom()).getBib() + " changed value to "
                            + sortedTimeList.get(c.getFrom()).getIgnoreTime());
                }
            }
        });
        // ///It is not this easy: CheckBoxTableCell does ot fire an onEditCommit action
        // //Need a change listener for the ignoreColumn. Find the cooked read, flag it
        // as an ignore, reprocess the bib
        // ignoreColumn.setOnEditCommit((CellEditEvent<CookedTimeData, Boolean> t) -> {
        // CookedTimeData ct =
        // t.getTableView().getItems().get(t.getTablePosition().getRow());
        // logger.debug("Ignore flag for CookedTime for bib " + ct.getBib() + " at " +
        // ct.getTimestamp().toString() + " is now " +
        // ct.ignoreTimeProperty().toString());
        ...
        // if there are no timing locations selected in the view then disable the entire
        // right hand side
        timingDetailsVBox.visibleProperty()
                .bind(timingLocListView.getSelectionModel().selectedItemProperty().isNull().not());
        ...
        assignToRaceToggleSwitch.visibleProperty()
                .bind(Bindings.size(RaceDAO.getInstance().listWaves()).greaterThan(1));
        assignToRaceToggleSwitch.managedProperty()
                .bind(Bindings.size(RaceDAO.getInstance().listWaves()).greaterThan(1));

        timingLocListView.getSelectionModel().getSelectedItems().addListener((Change<? extends TimingLocation> c) -> {
            logger.debug("timingLocListView changed...");
            // timingLocListView.getSelectionModel().getSelectedItems().forEach(System.out::println);
            ObservableList<TimingLocation> selectedTimingLocations = timingLocListView.getSelectionModel()
                    .getSelectedItems();
            ...
            if (selectedTimingLocations.isEmpty()) {
                ...
                if (selectedTimingLocation != null) {
                    timingLocationNameTextField.textProperty()
                            .unbindBidirectional(selectedTimingLocation.LocationNameProperty());
                    selectedTimingLocation = null;
                ...
                timingLocationNameTextField.textProperty()
                        .bindBidirectional(selectedTimingLocation.LocationNameProperty());
                ...
                if (selectedTimingLocation.getAutoAssignRaceID() < 0) {
                    ...
                    assignToRaceComboBox.getSelectionModel()
                            .select(selectedTimingLocation.autoAssignRaceProperty().getValue());
                }

                if (selectedTimingLocation.getIsAnnouncer())
                    announcerToggleSwitch.setSelected(true);
                else
                    announcerToggleSwitch.setSelected(false);

                // Show the filter start/end values
                filterEndTextField.textProperty()
                        .setValue(DurationFormatter
                                .durationToString(selectedTimingLocation.getFilterEndDuration(), 3, Boolean.TRUE)
                                .replace(".000", ""));
                filterStartTextField.textProperty()
                        .setValue(DurationFormatter
                                .durationToString(selectedTimingLocation.getFilterStartDuration(), 3, Boolean.TRUE)
                                .replace(".000", ""));

                logger.debug("Selected timing location is now " + selectedTimingLocation.getLocationName());
                // timingLocationDetailsController.setTimingLocationInput(null); //
                // .selectTimingLocation(selectedTimingLocations.get(0));
        ...
        timingLocationNameTextField.focusedProperty().addListener(
                (ObservableValue<? extends Boolean> arg0, Boolean oldPropertyValue, Boolean newPropertyValue) -> {
                    if (!newPropertyValue) {
                        ...
                        if (timingLocationNameTextField.getText().isEmpty()) {
                            ...
                            timingDAO.updateTimingLocation(
                                    timingLocListView.getSelectionModel().getSelectedItems().get(0));
                        }
        ...
        // Use this if you whant keystroke by keystroke monitoring.... Reject any non
        // digit attempts
        // filterStartTextField.textProperty().addListener((observable, oldValue,
        // newValue) -> {
        // if ( newValue.isEmpty() || newValue.matches("([0-9]*|[0-9]*:[0-5]?)") ) {
        // logger.debug("Possiblely good filter Time (newValue: " + newValue + ")");
        // } else if(newValue.matches("[0-9]*:[0-5][0-9]") ) { // Looks like a HH:MM
        // time, lets check
        ...
        filterStartTextField.focusedProperty().addListener(
                (ObservableValue<? extends Boolean> arg0, Boolean oldPropertyValue, Boolean newPropertyValue) -> {
                    if (!newPropertyValue) {
                        ...
                        if (DurationParser.parsable(filterStartTextField.getText())) {
                            ...
                            filterStartTextField.textProperty().setValue(DurationFormatter
                                    .durationToString(selectedTimingLocation.getFilterStartDuration(), 3, Boolean.TRUE)
                                    .replace(".000", ""));
                            return;
                        }
        ...
        filterEndTextField.focusedProperty().addListener(
                (ObservableValue<? extends Boolean> arg0, Boolean oldPropertyValue, Boolean newPropertyValue) -> {
                    if (!newPropertyValue) {
                        ...
                        if (DurationParser.parsable(filterEndTextField.getText())) {
                            ...
                            filterEndTextField.textProperty().setValue(DurationFormatter
                                    .durationToString(selectedTimingLocation.getFilterEndDuration(), 3, Boolean.TRUE)
                                    .replace(".000", ""));
                            return;
                        }
        ...
        filterStartTextField.textProperty().addListener((observable, oldValue, newValue) -> {
            ...
            if (newValue.isEmpty()
                    || newValue.matches("^[0-9]+(:([0-5]?([0-9]?(:([0-5]?([0-9]?(\\.\\d*)?)?)?)?)?)?)?")) {
        ...
        announcerToggleSwitch.selectedProperty().addListener(
                (ObservableValue<? extends Boolean> arg0, Boolean oldPropertyValue, Boolean newPropertyValue) -> {
        ...
        assignToRaceToggleSwitch.selectedProperty().addListener(
                (ObservableValue<? extends Boolean> arg0, Boolean oldPropertyValue, Boolean newPropertyValue) -> {
                    if (!newPropertyValue) { // Don't auto-assign
                        ...
                        // Do nothing. If they select a race to assign the runners to then we will do
                        // something.
                    }
                });
        assignToRaceComboBox.setItems(RaceDAO.getInstance().listRaces());
        assignToRaceComboBox.focusedProperty().addListener(
                (ObservableValue<? extends Boolean> arg0, Boolean oldPropertyValue, Boolean newPropertyValue) -> {
                    if (!newPropertyValue) {
                        TimingLocation tl = timingLocListView.getSelectionModel().getSelectedItems().get(0);
                        ...
                        if (tl != null && race != null) {
                            if (!tl.getAutoAssignRaceID().equals(race.getID())) {
                                logger.debug("Everybody at " + tl.getLocationName() + " now assigned to race "
                                        + race.getRaceName());
        ...
        // customChipBibCheckBox.selectedProperty().addListener((ObservableValue<?
            ...
            protected void updateItem(TimeOverride to, boolean empty) {
                ...
                if (empty || to == null || to.getBib() == null) {
                    ...
                    fullName.textProperty()
                            .bind(ParticipantDAO.getInstance().getParticipantByBib(to.getBib()).fullNameProperty());
                    ...
                    if (to.getNote().isEmpty())
                        toVBox.getChildren().setAll(nameHBox, timeHBox);
                    else {
        ...
        // overrideTableView.setPlaceholder(new Label("No overrides have been entered
        // yet"));
        ...
        // if (s== null) { return new SimpleStringProperty("Unknown: " +
        // cellData.getValue().getSplitId());
        // } else {
        ...
        // //return new
        // SimpleStringProperty(DurationFormatter.durationToString(cellData.getValue().getTimestamp(),
        // 3));
        // });
        //
        ...
        overrideRemoveButton.disableProperty()
                .bind(overrideListView.getSelectionModel().selectedItemProperty().isNull());
    ...
    public void resetTimingLocations(ActionEvent fxevent) {
        ...
        alert.setContentText(
                "This will reset the timing locations to default values.\nAll splits will be reassigned to one of the default locations.");
        // Label alertContent = new Label("This will reset the timing locations to
        // default values.\nAll splits will be reassigned to one of the default
        // locations.");
    ...
    public void addTimingLocation(ActionEvent fxevent) {
        ...
        logger.debug("Setting the timingLocListView.edit to " + timingLocationList.size() + " "
                + timingLocationList.indexOf(t));
    ...
    public void removeTimingLocation(ActionEvent fxevent) {
        ...
        if (splitsUsing.isEmpty().get()) {
            ...
            alert.setContentText(
                    "The timing location is in use by the following splits:\n" + splitsUsing.getValueSafe());
            alert.showAndWait();
        }
    }
    ...
    public void addTimingInput(ActionEvent fxevent) {
        ...
        tli.setLocationName("New " + selectedTimingLocation.getLocationName() + " Input "
                + (selectedTimingLocation.inputsProperty().size() + 1));
    ...
    private void showTimingInput(TimingLocationInput i) {
        ...
        FXMLLoader tlLoader = new FXMLLoader(
                getClass().getResource("/com/pikatimer/timing/FXMLTimingLocationInput.fxml"));
    ...
    public void clearAllTimes(ActionEvent fxevent) {
        ...
        alert.setContentText("Do you want to clear the times for all locations or just the "
                + selectedTimingLocation.getLocationName() + " location?");

        ButtonType allButtonType = new ButtonType("All Times");

        ButtonType currentButtonType = new ButtonType(selectedTimingLocation.getLocationName() + " Times",
                ButtonData.YES);
        ...
        if (result.get() == allButtonType) {
            ...
            timingLocationList.stream().forEach(tl -> {
            ...
            // ... user chose "Two"
            selectedTimingLocation.getInputs().stream().forEach(tli -> {
                tli.clearReads();
            });
        } else {
            // ... user chose CANCEL or closed the dialog
    ...
    public void setupCustomChipMap(ActionEvent fxevent) {
        ...
        try {
            ...
            logger.error("Error in setupCustomChipMap:", ex);
        }
    ...
    public void editOverride(TimeOverride o) {
        ...
        bibTextField.textProperty().addListener((observable, oldValue, newValue) -> {
            ...
            if (p != null) {
                ...
                if (p.wavesObservableList().size() > 1)
                    splitComboBox.setConverter(new StringConverter<Split>() {
                        @Override
                    ...
                    splitComboBox.setConverter(null);
        ...
        timeTextField.textProperty().addListener((observable, oldValue, newValue) -> {
            ...
            if (TimeOverrideType.OVERRIDE.equals(typeComboBox.getValue())) {
                if (relativeCheckBox.isSelected() && DurationParser.parsable(newValue))
                    timeOK.setValue(Boolean.TRUE);
                else if (!relativeCheckBox.isSelected() && DurationParser.parsable(newValue, false))
                    timeOK.setValue(Boolean.TRUE);
                else
                    timeOK.setValue(false);
            } else {
        ...
        splitComboBox.valueProperty().addListener((observable, oldValue, newValue) -> {
            if (splitComboBox.getSelectionModel().getSelectedItem() != null
                    && splitComboBox.getSelectionModel().getSelectedItem().getPosition() == 1) {
                relativeCheckBox.selectedProperty().setValue(Boolean.FALSE);
                relativeCheckBox.disableProperty().setValue(Boolean.TRUE);
            } else {
        ...
        if (o.getID() != null) {
            Platform.runLater(() -> {
                ...
                timeTextField.setText(DurationFormatter.durationToString(o.getTimestamp(), 3, Boolean.TRUE)
                        .replaceFirst("^(\\d:)", "0$1"));
                try {
                    ObservableList<Split> splitList = FXCollections.observableArrayList();
                    splitList.addAll(splitComboBox.getItems().stream().filter(e -> e.getID().equals(o.getSplitId()))
                            .findFirst().get());
                    splitComboBox.setItems(splitList);
                    splitComboBox.getSelectionModel().selectFirst();
                } catch (Exception e) {
            ...
            if (dialogButton == createButtonType) {
                ...
                if (TimeOverrideType.OVERRIDE.equals(typeComboBox.getValue())) {
                    ...
                    if (relativeCheckBox.isSelected())
                        ...
                        o.setTimestamp(DurationParser.parse(timeTextField.getText(), true));
        ...
        if (result.isPresent())
            timingDAO.saveOverride(result.get());
    ...
    public void startTriggerLookup(ActionEvent fxevent) {
        ...
        // Wrap it in a new list to avoid the list being modified while we stream/filter
        // it.
        List<CookedTimeData> cTimes = new ArrayList(timingDAO.getCookedTimes());
        List<CookedTimeData> cookedTimes = cTimes.stream().filter(c -> !c.ignoreTimeProperty().getValue())
        ...
        timingDAO.listTimingLocations().forEach(tl -> {
            ...
            List<Duration> startTimes = cookedTimes.stream()
                    .filter(p -> p.getRawChipID().equals("0") && p.getTimingLocationId().equals(tlID))
                    .map(c -> c.getTimestamp()).collect(Collectors.toList());
            startTimes.sort((p1, p2) -> p1.compareTo(p2));
            if (!startTimes.isEmpty())
                startTimesByLocation.put(tlID, FXCollections.observableArrayList(startTimes));
        });
        ...
        RaceDAO.getInstance().listRaces().forEach(race -> {
            ...
            if (wavesByLocation.containsKey(tlID))
                wavesByLocation.get(tlID).addAll(race.getWaves());
            else
                wavesByLocation.put(tlID, new ArrayList(race.getWaves()));
        ...
        wavesByLocation.keySet().forEach(tlID -> {
            ...
            if (wavesByLocation.keySet().size() > 1) {
                Label locLabel = new Label(
                        timingDAO.getTimingLocationByID(tlID).getLocationName() + " Timing Location");
            ...
            if (!startTimesByLocation.containsKey(tlID)) { // no start times
                ...
                if (size >= 6) {
                    waveTable.prefHeightProperty()
                            .setValue(waveTable.getFixedCellSize() * 5 + waveTable.getFixedCellSize() / 2);
                    waveTable.minHeightProperty()
                            .setValue(waveTable.getFixedCellSize() * 5 + waveTable.getFixedCellSize() / 2);
                    // waveTable.maxHeightProperty().setValue(waveTable.getFixedCellSize()*5 +
                    // waveTable.getFixedCellSize()/2);
                    ...
                    // waveTable.maxHeightProperty().setValue(1+(size + 1 )*
                    // waveTable.getFixedCellSize());
                    waveTable.maxHeight(Double.MAX_VALUE);
                }
                ...
                newTimeColumn.setCellFactory(ComboBoxTableCell.forTableColumn(new DurationStringConverter(),
                        startTimesByLocation.get(tlID)));
                ...
                for (int waveIndex = 0; waveIndex < maxWaveIndex; waveIndex++) {
                    ...
                    logger.debug("Looking for a start trigger for " + w.wName.get() + " at "
                            + DurationFormatter.durationToString(w.orgStartTime, 3));
                    ...
                    for (int i = startIndex; i < maxStartIndex; i++) {
                        ...
                        if (startTimes.get(i).equals(w.orgStartTime)) {
                            ...
                            startIndex = i + 1;
                            logger.debug(
                                    "  Exact Match at " + DurationFormatter.durationToString(w.newStartTime.get(), 3));
                            break;
                        }
                        ...
                        if (millis == null || startTimes.get(i).minus(d).abs().toMillis() < millis) {
                            ...
                            logger.debug(
                                    "  Possible Match of " + DurationFormatter.durationToString(startTimes.get(i), 3)
                                            + " within " + millis + " millis away");
                            // if the new time is closer to the next wave bail
                            if (waveIndex + 1 < maxWaveIndex && millis > waveStarts.get(waveIndex + 1).orgStartTime
                                    .minus(startTimes.get(i)).abs().toMillis()) {
                                logger.debug("  But it is closer to the next time of " + DurationFormatter
                                        .durationToString(waveStarts.get(waveIndex + 1).orgStartTime, 3));
                                logger.debug("  which is only " + waveStarts.get(waveIndex + 1).orgStartTime
                                        .minus(startTimes.get(i)).abs().toMillis() + " millis away");
                                break;
                            }
                            w.newStartTime.set(startTimes.get(i));
                            w.update.set(true);
                            ...
                            logger.debug(
                                    "  Possible Match of " + DurationFormatter.durationToString(startTimes.get(i), 3)
                                            + " is " + startTimes.get(i).minus(d).abs().toMillis() + " millis away");
                            break;
                        }
                    }
                }
        ...
        if (result.isPresent()) {
            ...
            results.forEach(k -> {
                if (k.update.get()) {
                    k.w.setWaveStart(
                            (LocalTime.MIDNIGHT.plus(k.newStartTime.get())).format(DateTimeFormatter.ISO_LOCAL_TIME));
    ...
    private void updateFilterPredicate() {
        filteredTimesList.setPredicate(cookedRead -> {
            ...
            if (searchTextBox.textProperty().getValueSafe().isEmpty()
                    && searchScopeCheckComboBox.getCheckModel().getCheckedItems().isEmpty()) {
            ...
            if (!searchScopeCheckComboBox.getCheckModel().getCheckedItems().isEmpty()) {
                searchScopeCheckComboBox.getCheckModel().getCheckedItems().forEach(tl -> {
                    if (tl != null && tl.getID().equals(cookedRead.getTimingLocationId()))
            ...
            try {
                ...
                if (p == null) {
                    if (cookedRead.getBib().startsWith("Unmapped"))
                        name = "Unknown Chip";
                    else if (cookedRead.getRawChipID().equals("0"))
                        name = "START TRIGGER";
                    else
                        name = "Unregistered bib: " + cookedRead.getBib();
                } else {
                    name = StringUtils.stripAccents(p.fullNameProperty().get());
                }

                if (pattern.matcher(name).matches() || pattern.matcher(cookedRead.getRawChipID()).matches()
