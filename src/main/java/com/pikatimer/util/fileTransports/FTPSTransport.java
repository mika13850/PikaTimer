...
public class FTPSTransport implements FileTransport {
            ...
            public Void call() {
                ...
                while (true) {
                    try {
                        ...
                        while (true) {
                            ...
                            while (fatalError || ftpClient == null || !ftpClient.isConnected()) {
                                ...
                                if (!ftpClient.isConnected()) {
                                    logger.debug(
                                            "FTPSTransport Thread: Still not connected, sleeping for 10 seconds...");
                                    Thread.sleep(10000);
                                }
                            ...
                            Platform.runLater(() -> {
                                if (encrypted && protMode.equalsIgnoreCase("P"))
                                    transferStatus.set("Transfering (Secure) " + fn);
                                else if (encrypted)
                                    transferStatus.set("Transfering (Clear) " + fn);
                                else
                            ...
                            try {
                                ...
                                logger.error("ftpClient.rename exception thrown", ex);

                                try {
                                    ftpClient.dele(fn);// This may fail if the file does not exist
                                } catch (Exception ex2) {
                                    // noop
                                    logger.error("ftpClient.dele exception thrown", ex2);
                                }
                                ftpClient.rename(tmpFn, fn);
                            }
                            ...
                            logger.debug("FTPSTransport Thread: transfer of " + filename + " done in "
                        ...
                        logger.error("FTPSTransport Thread: InterruptedException thrown", ex);
                        // if (filename!= null) transferQueue.put(filename);

                        // Logger.getLogger(FTPSTransport.class.getName()).log(Level.SEVERE, null, ex);
                    } catch (IOException ex) {
                        logger.error("FTPSTransport Thread: IOException thrown", ex);
                        ...
                        logger.error("FTPSTransport Thread: Exception tossed", ex);
    ...
    private void openConnection() {
        try {
            ...
            Platform.runLater(() -> {
                if (encrypted)
                    transferStatus.set("Connecting FTPS...");
                else
                    transferStatus.set("Connecting...");
            });
            ...
            if (FTPReply.isPositiveCompletion(reply)) {
                ...
                Platform.runLater(() -> {
                    if (encrypted)
                        ...
                        transferStatus.set("Loging in (insecure)...");
                });
                ...
                if (ftpClient.login(username, password)) {
                    ...
                    Platform.runLater(() -> {
                        transferStatus.set("Changing Directories...");
                    });
                    ...
                    Platform.runLater(() -> {
                        transferStatus.set("Error: Login Failed");
                    });
                    fatalError = true;
                    try {
                        ftpClient.disconnect();
                ...
                Platform.runLater(() -> {
                    transferStatus.set("Error: Unable to connect to host");
                });
            ...
            if (encrypted) {
                ...
                Platform.runLater(() -> {
                    transferStatus.set("Error: Network Error");
                });
            }
    ...
    public void save(String filename, String contents) {
        ...
        // if (transferThread == null || ftpClient == null || !ftpClient.isConnected() )
        // transferFile(); // kicks off the thread
    }
    ...
    public void refreshConfig() {
        try {
            ...
            logger.error("FTPSTransport::refreshConfig Exception!", e);
            fatalError = true;
    ...
    public void test(ReportDestination parent, StringProperty output) {
        Task transferTask = new Task<Void>() {
            ...
            public Void call() {
                ...
                Platform.runLater(
                        () -> output.set(output.getValueSafe() + "Connecting to " + hostname + " via FTPS..."));
                ...
                try {
                    ...
                    try {
                        ...
                        try {
                            ...
                            logger.error("Unexpected exception", ex2);
                        }
                        return null;
                    }
                }
                ...
                if (FTPReply.isPositiveCompletion(reply)) {
                    ...
                    try {
                        ...
                        logger.error("IO exception when disconnecting", ex);
                    }
                    return null;
                }
                try {
